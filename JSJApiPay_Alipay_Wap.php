...
function JSJApiPay_Alipay_Wap_config() {
    $configarray = array(
		"FriendlyName" => array("Type" => "System", "Value"=>"金沙江[支付宝WAP移动端]免签 即时到账API接口 For WHMCS - Code By Tutugreen.com"),
...
function JSJApiPay_Alipay_Wap_link($params) {
	...
	*/
	...
	"addnum"        => trim($JSJApiPay_Alipay_Wap_config['addnum']),
	"amount"        => trim($amount),
	"return_url"	=> trim($JSJApiPay_Alipay_Wap_config['return_url']),
	"invoiceid"		=> trim($invoiceid),
	"apiid"		    => $JSJApiPay_Alipay_Wap_config['apiid'],
	"apikey"		=> strtolower(md5($JSJApiPay_Alipay_Wap_config['apikey'])),
	"api_url"		=> trim($JSJApiPay_Alipay_Wap_config['api_url']),
	);

	$html_code="
	<!-- Powered By api.jsjapp.com , Coded By Tutugreen.com -->
	<form name='JSJApiPay_Alipay_Wap_form' action='".$parameter['api_url']."' method='POST'>
	...
	<a href='#' onclick=\"document.forms['JSJApiPay_Alipay_Wap_form'].submit();\"><img src='$img' alt='点击使用支付宝支付'> </a>
	";
	/**备用：<a href='#' onclick=\"document.forms['JSJApiPay_Alipay_Wap_form'].submit();\"><img src='$img' alt='点击使用支付宝支付'> </a>**/

	if ($debug) {
		$msg="[JSJApiPay_Alipay_Wap]订单: $invoiceid 生成支付表单 $html_code";
